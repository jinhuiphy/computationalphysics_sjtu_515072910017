function y = Wave1DFD_right_free(t,x,y,yt,c)
%
% Wave1DFD numerically solves the one-dimensional wave
% equation, with free left end, using the
...
% the 2. initial position for the left end
y(end,2) = (1-r/2)*y(end,1) + r*y(end,1)/2 - dt*yt(end);
% the 2. initial position for the interior points
m = 2:length(x)-1;
y(m,2) = (1-r)*y(m,1) + r/2*(y(m-1,1) + y(m+1,1)) + dt*yt(m);

% time stepping 
for n = 2:length(t)-1
    y(end,n+1) = (2-r)*y(end,n) - y(end,n-1) + r*y(end-1,n);
